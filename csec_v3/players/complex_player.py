...
def final_player(state):
    return state.turn_idx == ((state.leader_idx + 3) %4)

def points_on_table(state):
    return sum([c.card_points() for c in state.trick if c]) == 0 
...
def check_AKQH(state):
    for player in range(4):
        if player != state.turn_idx:
            for card in state.player_hands[player]:
                if card.suit == Suit.hearts and (card.rank == Rank.ace or card.rank == Rank.king or card.rank == Rank.queen):
                    return False
    return True

def check_large_heart_in_hand_and_hist(state):
    count = 0
    for round in range(state.trick_nr):
        card = state.history[round][state.turn_idx]
        if card.suit == Suit.hearts and card.rank.value >= Rank.seven.value:
            count += 1
...
def check_highest_heart(state):
    m = max_suit(state, Suit.hearts)
    for player in range(4):
        if player != state.turn_idx:
            for card in state.player_hands[state.turn_idx]:
                if card.suit == Suit.hearts:
                    if not m or m.rank.value < card.rank.value:
                        return False
    return True

def safe_hands(state):
    if state.suit == None:
...
def unsafe_hands(state):
    if state.suit == None:
        raise ValueError("This cannot be calculated if we are at the leader")

    m = max([t for t in state.trick if t and t.suit == state.suit])
    unsafe_hands = [t for t in state.legal_moves() if t.rank > m.rank]
    return unsafe_hands
...
def max_safe_hand(state):
    ...
    if x == None:
        ...
        return max(x, default = None, key = lambda c : c.rank)

def max_unsafe_hand(state):
    x = unsafe_hands(state)
    if x == None:
        return None
    else:
        return max(x, default = None, key = lambda c : c.rank)
...
def find_card_in_trick(state, list_of_cards):
    for find_card in list_of_cards:
        for card in state.trick:
            if card and card == find_card:
                return card
    return None
...
def check_STM(state):
    if sum([state.player_pts[i] for i in range(4) if i != state.turn_idx]) != 0:
        return False
    if not check_AKQH(state):
        return False
    if not check_large_heart_in_hand_and_hist(state):
        return False
    if not check_highest_heart(state):
        return False
    if find_card(state, [Card(Suit.spades, Rank.queen),Card(Suit.spades, Rank.ace),Card(Suit.spades, Rank.king)]) or check_QS_gone(state):
        #print(state)
        return True
    return False


def STM_leader(state):
    x = find_card_in_legal_moves(state, [Card(Suit.spades, Rank.queen),Card(Suit.spades, Rank.ace),Card(Suit.spades, Rank.king)]) 
    if x:
        return x
    x = max_suit(state, Suit.hearts)
...
def STM_follower(state):
    if state.suit == Suit.spades and Card(Suit.spades, Rank.queen) in unsafe_hands(state):
        return Card(Suit.spades, Rank.queen)

    if points_on_table(state):
        return max_unsafe_hand(state)

    if state.suit == Suit.spades:
        if final_player(state):
            return min_excl_qs(state)
        else:
            if find_card_in_trick(state, [Card(Suit.spades, Rank.ace)]):
                return min_excl_qs(state)
            elif find_card_in_trick(state, [Card(Suit.spades, Rank.king)]):
                x = find_card_in_legal_moves(state, [Card(Suit.spades, Rank.ace)])
                if x:
                    return x
                return min_excl_qs(state)
        ...
        if not KV_Clubs and RC[0] < 2 and not KV_Diamonds and RC[1] < 2:
            return max_card(state)
        return min_card(state)


class MoreComplexPlayer(Player):
    def __init__(self):
        pass

    def __str__(self):
        return "Complex Heuristic Player, No Cheating"
    ...
    def __call__(self, state):
        ...
        if state.leader_idx == state.turn_idx:
            ...
            if points_on_table(state) and final_player(state):
                x = max_excl_qs(state)
                if x:
                    return x
...
def STM_follower_cheating(state):
    if state.suit == Suit.spades and Card(Suit.spades, Rank.queen) in unsafe_hands(state):
        return Card(Suit.spades, Rank.queen)
    ...
    if state.suit == Suit.spades:
        if final_player(state):
            ...
            if find_card_in_trick(state, [Card(Suit.spades, Rank.ace)]):
                if playersbehind_check_qs(state):
                    return None
                return min_excl_qs(state)
            elif find_card_in_trick(state, [Card(Suit.spades, Rank.king)]):
                if playersbehind_check_qs(state):
                    return find_card_in_legal_moves(state, [Card(Suit.spades, Rank.ace)])
                return min_excl_qs(state)
    

        if not playersbehind_check_opp_void_or_has_no_points(state):
            return max_card(state)
        return min_card(state)


class ComplexCheatingPlayer(Player):
    def __init__(self):
        pass

    def __str__(self):
        return "Complex Heuristic Player, with Cheating"
    ...
    def __call__(self, state):
        ...
        if state.leader_idx == state.turn_idx:
            ...
            if points_on_table(state) and playersbehind_check_opp_void_or_has_no_points(state):
